...
import * as techniquesService from '../services/stress-techniques.service.js';
...
export async function createTechnique(req: Request, res: Response) {
  const technique = await techniquesService.createTechnique(req.params.id, req.body as StressTechniqueInput);
  return ok(res, { technique }, 201);
}

export async function updateTechnique(req: Request, res: Response) {
  const technique = await techniquesService.updateTechnique(req.params.techId, req.body as StressTechniqueInput & { audio_url?: null });
