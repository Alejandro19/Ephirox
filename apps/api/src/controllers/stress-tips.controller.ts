...
import type { StressTipInput, StressTipUpdate } from '@latribu/shared-types';
import * as tipsService from '../services/stress-tips.service.js';
...
export async function createTip(req: Request, res: Response) {
  const { content } = req.body as StressTipInput;
...
export async function updateTip(req: Request, res: Response) {
  const tip = await tipsService.updateTip(req.params.tipId, req.body as StressTipUpdate);
