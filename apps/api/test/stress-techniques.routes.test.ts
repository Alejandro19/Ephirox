...
import { clients, stressTechniques, clientNotifications } from '../src/models/schema.js';
import { signToken } from '../src/services/auth.service.js';

describe('stress techniques routes', () => {
  const app = createApp();
  ...
  beforeAll(async () => {
    const [client] = await db
      .insert(clients)
      .values({ name: 'Stress Client', email: `stress-${Date.now()}@example.com`, status: 'active', clientType: 'coaching_1_1' })
      .returning();
    clientId = client.id;
  ...
  afterEach(async () => {
    await db.delete(stressTechniques).where(eq(stressTechniques.clientId, clientId));
  });

  it('a client with permissions.stress=true and no techniques yet gets an empty list', async () => {
    await db.update(clients).set({ permissions: { stress: true } }).where(eq(clients.id, clientId));
    const res = await request(app).get(`/api/clients/${clientId}/stress-techniques`).set('Authorization', `Bearer ${clientToken}`);
  ...
  it('rejects a client from assigning their own technique (admin-only)', async () => {
    const res = await request(app)
      .post(`/api/clients/${clientId}/stress-techniques`)
      .set('Authorization', `Bearer ${clientToken}`)
      .send({ title: 'Respiración 4-7-8' });
    expect(res.status).toBe(403);
  });

  it('admin assigns a technique, which unlocks the stress module and notifies the client', async () => {
    const res = await request(app)
      .post(`/api/clients/${clientId}/stress-techniques`)
    ...
    expect((updatedClient.permissions as Record<string, boolean>).stress).toBe(true);
  ...
  it('creates a technique flagged as a Rox Ritual, and a later partial edit does not clear the flag', async () => {
    ...
    const updateRes = await request(app)
      .put(`/api/clients/${clientId}/stress-techniques/${techId}`)
      .set('Authorization', `Bearer ${adminToken}`)
  ...
  it('admin updates and deletes a technique', async () => {
    ...
    const deleteRes = await request(app).delete(`/api/clients/${clientId}/stress-techniques/${techId}`).set('Authorization', `Bearer ${adminToken}`);
  ...
  it('uploads a video and attaches it to the technique', async () => {
    ...
    const res = await request(app)
      .post(`/api/clients/${clientId}/stress-techniques/${techId}/upload`)
      .set('Authorization', `Bearer ${adminToken}`)
  ...
  it('uploads audio, then replacing it deletes the old file via storage and stores the new one', async () => {
    ...
    const firstUpload = await request(app)
      .post(`/api/clients/${clientId}/stress-techniques/${techId}/upload-audio`)
      .set('Authorization', `Bearer ${adminToken}`)
