...
export default function AdminNotificationsPanel() {
  ...
  if (loading) return <p style={{ color: "var(--ink-secondary)", fontSize: 14 }}>Cargando…</p>;
    ...
    <div>
      ...
      <div style={{ borderTop: "1px solid var(--border-hairline)" }}>
        {notifications.length === 0 ? (
          <p style={{ textAlign: "center", color: "var(--ink-secondary)", fontSize: 13,
            padding: "32px 0" }}>No hay notificaciones por ahora.</p>
        ) : (
          notifications.map((n) => (
            <div key={n.id} style={{ display: "flex", alignItems: "center",
              justifyContent: "space-between", padding: "14px 20px",
              borderBottom: "1px solid var(--border-hairline)", opacity: n.read ? 0.6 : 1 }}>
              <div style={{ flex: 1 }}>
                ...
                {n.client_id && (
                  <a href={`/admin/clients/${n.client_id}`}
                    style={{ padding: "5px 14px", borderRadius: "9999px",
                      border: "1px solid var(--border-hairline)", fontSize: 11, fontWeight: 500,
                ...
                {!n.read && (
                  <button onClick={() => markRead(n.id)}
                    style={{ padding: "5px 14px", borderRadius: "9999px",
                      ...
                      fontSize: 11, fontWeight: 500, color: "var(--ring-accent)",
