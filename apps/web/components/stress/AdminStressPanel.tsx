...
import { STRESS_TECHNIQUE_TYPES } from '@latribu/shared-types';

const TECHNIQUE_TYPES: readonly string[] = STRESS_TECHNIQUE_TYPES;
...
type EditDraft = {
  title: string; type: string; minutes: string; seconds: string; youtubeUrl: string; description: string; precautionNote: string; isRitual: boolean;
};

function draftFromTechnique(t: StressTechnique): EditDraft {
  return {
    title: t.title || '',
...
export function AdminStressPanel({ clientId }: { clientId: string }) {
  const { data: techniques = [] as StressTechnique[], error: loadError, isLoading: loading, mutate } = useSWR(
    ['stress-techniques', clientId],
  ...
  function startEdit(t: StressTechnique) {
    setEditingId(t.id);
