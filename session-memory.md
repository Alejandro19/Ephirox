...
### Actividad 2 — Migrar el NFC del gimnasio a la infraestructura real nueva (ya no bloqueada, pendiente de ejecutar)

- Ya se resolvió la decisión (sesión 2026-09-02→09-03): deploy real a Vercel (`ephirox.com`) + Railway (`api.ephirox.com`), con dominio propio comprado en Hostinger — ver el resumen ejecutivo de esa sesión. Falta confirmar si el NFC físico del gimnasio ya apunta a esta infraestructura nueva o todavía depende del túnel de Cloudflare viejo — si sigue en el túnel, hay que reprogramarlo hacia el dominio real cuanto antes, ya que el túnel puede caerse/cambiar de URL en cualquier reinicio.
