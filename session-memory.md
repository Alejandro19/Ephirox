...
## Resumen Ejecutivo — Sesión 2026-09-12 → 2026-09-13 — Sección "Optimización personalizada", tramo "punto ciego"/"coste para la empresa", y rediseño completo de "Junta directiva"
...
### 6. Ajustes sueltos de continuidad (2026-09-12, tarde)

Chips "ya lo intentaste" no cabían en una sola fila en mobile a 26px — se resolvió con `flex-wrap:nowrap` + tamaño/padding en `clamp()` (baja hasta ~11-19px en mobile) en vez de dejar que envuelvan a 2 líneas. De paso se encontró y corrigió una colisión real de nombre de clase: `.chip` ya existía (con otro diseño) para el selector de tamaño de equipo del formulario de leads — se renombró la de los chips tachados a `.diagnostico-chip` para no pisarse. También: header móvil forzado a una sola fila junto al logo (se quitó un `flex-wrap`/`flex-basis:100%` que partía "Entrar"/"Llevar a mi empresa" a una segunda línea — cambio de criterio explícito de Alejandro, revirtiendo una decisión de una sesión anterior), y la palabra "vuelve" en cursiva (`.cambia-head h2 em`) tuvo que ajustarse dos veces: bajarla de `1.12em` (se veía más grande que el resto) a `1em` no bastó (Cormorant Garamond tiene un x-height notablemente menor que Jost, la fuente base, así que a igual `font-size` se sigue leyendo más chica) — el valor final que sí igualó el tamaño visual fue `1.08em`.

### 7. Rediseño completo de "El reporte que le llevas a tu Junta" (10a) — sesión 2026-09-13

Prompt completo (`docs/EPHIROX - Prompt Seccion Junta 10a.md`) reemplazó el acordeón (que repetía el patrón de DÍA 90) por un índice + panel lateral de **altura fija** (nunca acordeón — el alto no puede cambiar al pasar de un punto a otro o empuja la card clara que viene después). Cada uno de los 5 puntos tiene su propia forma de gráfico en vez de la misma serie de barras repintada: cohorte (12 barras, 2 resaltadas), comparación (dos barras horizontales, reutilizada en 2 puntos), línea de tiempo (relleno con degradado), ciclos (28 marcas). Fila activa del índice: filete dorado a la izquierda + sangría de 18px + contraste de color, sin fondo ni flechas de acordeón. Accesible (`role="tab"`/`"tabpanel"`, `aria-selected`/`aria-controls`).

**Pedido explícito de marca, con efecto retroactivo:** las cifras grandes de cada panel debían usar el mismo estilo de referencia que `.cost-card .num` (Cormorant Garamond 600, `#E4A64A`, tabular-nums) en vez de la tipografía genérica que traía el prompt — documentado en memoria (`feedback_landing-numeros-graficas.md`) como regla permanente para cualquier cifra destacada nueva en la landing, de ahora en adelante.

**Bug real encontrado por Alejandro con una captura:** `JuntaChart.tsx` renderizaba su propio `<section className="junta-section">` DENTRO del `<section className="junta-section">` que ya lo envuelve en `LandingPage.tsx` — dos `border-top` apilados producían una segunda línea divisoria flotando en el espacio vacío entre DÍA 90 y el título de Junta. Se quitó el `<section>` interno. De paso, "CINCO PUNTOS DEL ACTA" se reemplazó por "Selecciona cada punto para ver el detalle." (misma clase `.rows-hint` con el ↳ dorado que ya usa DÍA 90, reposicionado arriba del índice) — pedido explícito de usar el mismo patrón que la sección vecina, aunque el prompt original pedía justo lo contrario (quitar ese texto para no repetir el eco de DÍA 90).

**Punto 02 ("Retorno que se paga solo") tuvo la iteración más larga**, en 3 rondas de feedback sucesivas:
1. El "1:1" no se explicaba solo (a diferencia de 5X/10X) y la comparación de 2 barras mezclaba un dato real citado (30–50% del salario anual) con una barra "ilustrativa" sin fuente que visualmente tenía la misma autoridad — además comparaba el costo de reemplazar una persona con el costo del programa de toda una cohorte sin aclarar que son escalas distintas. Se reemplazó todo el bloque (texto + cifra + barras) por solo una cifra grande "30–50%" + glosa + una fuente real (la misma ya citada en la card 5X de "El costo") — más simple, más defendible. Esto requirió agregar `chart: 'none'` al tipo `JuntaPunto` (puntos sin gráfico) y un campo opcional `fuente`.
2. La fuente bibliográfica quedaba alineada con la cifra grande en vez de con la glosa (el texto más cercano) — se movió al mismo contenedor flex que la glosa (`.junta-glosa-col`) para que compartan el mismo margen izquierdo.
3. La cifra grande (40-48px) y la glosa (13px) usaban `align-items:baseline`, lo que alineaba por línea de base de texto y dejaba la cifra empezando visualmente más arriba que el párrafo de al lado — cambiado a `align-items:start` para que ambos arranquen del mismo borde superior, "más uniforme".

**Pedido de seguimiento sobre la card "¿Cómo opera el sistema?" (`.dif-card`):** primero se pidió un diseño "similar pero no igual" al panel de Junta (se igualó `border-radius` a 14px y se afinó el borde a 1px translúcido, manteniendo su degradado dorado-crema original) — pero en un turno posterior Alejandro aclaró que quería el tono efectivamente **oscuro** como el panel de Junta, no solo el radio compartido. Se reemplazó el degradado cálido por el mismo fondo translúcido oscuro que usa `.junta-panel` y se recolorearon todos los textos/acentos que asumían fondo claro (texto rotativo, eyebrow, filete superior, dots, barra de progreso) a tonos claros/dorados legibles sobre oscuro. **Lección: un pedido de "diseño similar" puede evolucionar en varios turnos — no asumir que la primera interpretación (conservar la identidad de color propia) es la final; confirmar visualmente en cada ronda de feedback antes de dar el ajuste por cerrado.**

**Fricción de tooling repetida esta sesión:** con varios procesos de Chrome headless abiertos en paralelo (uno por verificación), los scripts que buscan la pestaña por `url.includes('/landing')` a veces enganchan una pestaña vieja/equivocada en vez de la recién creada, y los `querySelector` corridos justo después de un `.click()` a veces leen el DOM antes de que React re-renderice (mismo síntoma: la captura o el texto extraído corresponden al punto anterior, no al recién clickeado). Mitigación que funcionó de forma consistente: matar TODOS los procesos de Chrome headless entre rondas de verificación (`pkill -f "remote-debugging-port=94"`) y usar `document.querySelectorAll(...)[i].click()` seguido de un `await` de ≥500ms antes de leer cualquier texto o tomar la captura, en vez de encadenar click+scroll+captura sin pausas intermedias.

### 8. Commits (todos en `main`, `origin/Alejandro19/Ephirox`)
...
- Dos ajustes puntuales de copy: la segunda frase del array `DIF` (card rotativa "¿Cómo opera el sistema?") pasó de "Es optimización de hábitos y rendimiento, no diagnóstico ni tratamiento." a "...hábitos, salud y alto rendimiento..." (commit `a2bf6df` — se dejó sin tocar a propósito una frase parecida pero con redacción distinta en el footer, "Programa de optimización de hábitos y rendimiento...", por no ser el mismo texto literal pedido); y el subtítulo del hero (`.lead`) pasó de "...antes de que pase factura." a "...antes de que **te** pase factura." (commit `b68d6e9`).

### 10. Continuación 2026-09-13 — header mobile con ícono, rediseño completo del formulario "Llevar Ephirox a mi empresa", y SEO (title/description)

Tanda larga de fixes/pedidos encadenados sobre la landing, mismo patrón de verificación (`tsc` + CDP en 390/768/1440px) y push solo tras confirmación explícita:

- **Chips "ya lo intentaste" seguían poco legibles** aun tachados en mobile — se subió el tamaño de fuente (`clamp(11px,3.2vw,26px)` → `clamp(13px,3.6vw,26px)`) y la opacidad del color (0.4→0.5), verificado con medición real de `getBoundingClientRect` a 375/390px que las 3 chips siguen cabiendo en una sola línea con margen de sobra.
- **Header mobile: "Entrar →" muy pegado al botón "Llevar a mi empresa".** Alejandro no estaba seguro entre separar más el texto (quitando la flecha) o usar un ícono — se le preguntó explícitamente con `AskUserQuestion` y eligió **ícono de login** (SVG inline nuevo, `LoginIcon`, con `aria-label="Iniciar sesión"` en el link para no perder el texto accesible). Con el texto largo fuera, se pudo además subir el gap entre el link y el botón (18px→28px en mobile). Commit `1802a7b`.
- **Rediseño completo de la sección "Llevar Ephirox a mi empresa"** siguiendo un prompt detallado (no versionado en el repo): tarjeta partida en dos mitades — izquierda oscura (`#1E1811`) con filete dorado + h2 + párrafo centrados verticalmente (`align-content:center`, clave para que no quede pegado arriba), derecha crema (`#EFE8DA`) a sangre con el formulario (labels uppercase visibles, inputs con solo borde inferior, chips de tamaño de equipo con "11–30" preseleccionado y `aria-pressed`, botón ancho completo "Preparar la propuesta"). Commit `1802a7b`.
  - **Bug real encontrado y corregido durante la verificación (no reportado por Alejandro, detectado probando el flujo real):** las 3 CTAs "Llevar Ephirox a mi empresa" (header/hero/footer) hacen `scrollIntoView({block:'start'})` sin compensar el header fijo — con el padding más ajustado que pedía el prompt, en anchos donde ese padding caía cerca del piso del `clamp` (~768px) el header terminaba tapando el borde superior de la tarjeta clara al hacer clic en cualquiera de esas 3 CTAs. Se agregó `scroll-margin-top:90px` a `section.llevarlo`, confirmado con un clic real simulado (no solo `scrollIntoView` directo) en 390/768/1440px.
- **"Meta título y meta description del sitio" (SEO/share-preview) — 3 iteraciones del mismo pedido**, cada vez con texto aprobado distinto, en `apps/web/app/landing/page.tsx` (`TITLE`/`DESCRIPTION`, usadas también en `openGraph`/`twitter`): terminó en Title "Ephirox — Bienestar élite para Founders y C-Levels" / Description "Plataforma de bienestar para talento de alto impacto y top sellers — salud y rendimiento sostenido sin burnout, potenciados por IA." Commits `487284a`, `f5b476c`, `be3d5b1`, `afa068b`. Se dejó sin tocar a propósito `opengraph-image.tsx` (la imagen visual de share, con su propio tagline "Redefining limits.") por no ser parte explícita del pedido (solo etiquetas de texto `<head>`).
- **Footer:** "Programa de optimización de hábitos y rendimiento." → "...y alto rendimiento." (commit `85944bd`).
- **Tarjeta del formulario reportada como "muy alta" en mobile (~997px, casi 1.2 viewports) → 3 rondas de compactado:**
  1. Correo/Celular y Empresa/Rol (cada uno 100% de ancho en mobile por el `flex-basis` en px pensado para escritorio) se agruparon de a 2 por fila también en mobile, con `flex-basis` en % en vez de px. El primer split 50/50 recortaba el placeholder de Correo; se midió con `canvas.measureText` el ancho real necesario de cada placeholder contra el ancho real de columna disponible (no a ojo) y se ajustó a 53/47. `wrapH` bajó de ~997px a ~754px. Commit `726aef4`.
  2. **Con un iPhone real, Alejandro reportó dos problemas que el test con placeholders no había mostrado:** el campo Celular se pintaba de amarillo al escribir (autofill nativo de Safari en inputs `tel`/`email`, sin que el usuario aceptara ningún autofill — se corrigió con el truco estándar `input:-webkit-autofill { box-shadow: 0 0 0 1000px #EFE8DA inset; -webkit-text-fill-color: var(--ink); }`), y Correo se veía apretado (un correo real como el suyo es más largo que el placeholder usado para medir, así que el margen calculado en la ronda 1 no alcanzaba en la práctica). Se separaron las clases (`field-correo`/`field-celular` vs `field-180`/`field-150`) para volver Correo/Celular a ancho completo en mobile sin afectar el par Empresa/Rol, que sí quedó bien. Commit `6f12147`. **Lección: medir el ancho necesario contra el placeholder de diseño no es lo mismo que contra contenido real tecleado por un usuario — para campos de longitud variable (correo, nombre) conviene dejar ancho completo en mobile en vez de perseguir el ajuste exacto de un placeholder de ejemplo.**
  3. Se ocultó el párrafo "Te ayudamos a preparar la propuesta que se aprueba." de la mitad izquierda, solo en mobile, para ganar espacio vertical (el h2 solo ya comunica el propósito). Commit `cb5152b`.

### 11. Continuación 2026-09-15 — SEO técnico (redirect + noindex), LinkedIn en el footer, formulario "propuesta"→"demo" con migración real, y kicker nuevo del hero

- **SEO técnico en `apps/web/middleware.ts`** (no en `next.config.ts`, para no afectar `app.ephirox.com` sin querer): `ephirox.com/landing` pasa de servirse igual que `/` (contenido duplicado indexable dos veces) a un **301 a `/`** — implementado dentro de la rama de host de marketing, ya existente, en vez de usar `redirects()` de Next porque ese archivo no tiene forma nativa de aplicar solo a un host. Además, `app.ephirox.com` (el producto — login, dashboard, NFC) ahora manda `X-Robots-Tag: noindex, nofollow` en TODAS sus respuestas (incluidas las que redirigen a `/login`), para que no aparezca en Google a diferencia del dominio de marketing. Verificado con `curl -H "Host: ..."` spoofeado contra ambos dominios. Commits `3f1413f`, `146ce6d`.
- **Footer:** nuevo item "LinkedIn" (`linkedin.com/company/146593282`, texto de link "Ephirox" por no tener slug de vanidad) arriba de Instagram. Commit `fe0c757`.
- **Ícono de login del header** (agregado la sesión anterior) pasó de flecha-en-puerta a un ícono de persona real (círculo + arco de hombros), pedido con una imagen de referencia adjunta — luego ajustado de 18px a 20px para quedar al mismo tamaño que el naming "EPHIROX". Commits `4ef257c`, `17ccd2c`.
- **Formulario "Llevar Ephirox a mi empresa": de "preparar propuesta" a "solicitar demo".** Cambio de producto, no solo de copy — la calificación del lead (empresa/rol/tamaño de equipo) se mueve de "antes, por formulario" a "en vivo, en la llamada". Se redujo el formulario a Nombre/Correo/WhatsApp (renombrado de "Celular"), se quitaron Empresa/Rol/chips de tamaño/"¿Quién más...", botón → "Solicitar una demo". **Esto rompía el envío real:** `empresa`/`rol` eran `NOT NULL` tanto en el zod schema compartido (`@latribu/shared-types`) como en la columna real de Postgres — se relajaron ambos a opcionales/nullable (sin borrar las columnas, quedan disponibles para completarse a mano durante la llamada, pedido explícito de Alejandro). El ALTER TABLE contra la base real fue bloqueado por el clasificador de auto mode (mismo patrón que otras veces esta sesión con cambios de schema) y se corrió recién con confirmación explícita vía `AskUserQuestion`, contra `DATABASE_URL` (producción) y `TEST_DATABASE_URL`, con un script `tsx`/`postgres` desechable borrado al terminar. El mensaje de la alarma admin y el correo interno (`enterprise-leads.service.ts`) se reescribieron para no asumir que `empresa`/`rol` vienen. Se probó con un envío real contra la API (201) y se borró el lead de prueba de producción después. Commit `93f28cf`.
- **Kicker nuevo sobre el H1 del hero** — "PLATAFORMA DE BIENESTAR ÉLITE PARA FOUNDERS Y C-LEVELS" inicialmente, acortado después a **"BIENESTAR ÉLITE PARA FOUNDERS Y C‑LEVELS"** (quitar "Plataforma de" — redundante con el meta description y más corto, a tono con el resto de kickers del sitio). En mobile angosto el guion normal de "C-Levels" se partía como punto de quiebre válido de línea (comportamiento normal del navegador, no bug) — se resolvió con guion no-separable U+2011 más un `<span style="white-space:nowrap">` de refuerzo. `.hero-content` es `display:grid` con gap parejo entre hijos; el kicker usa `margin-bottom` negativo para cancelar ese gap y quedar pegado al h1 (4px en escritorio, ~18px en mobile — pedido explícito en dos rondas separadas). Se aprovechó para también reemplazar el texto "¿Ya eres miembro? Iniciar sesión →" por el mismo ícono de persona en escritorio (antes solo aplicaba <=680px). Commits `dc901f4`, `935dda0`.
- **Se eliminó el link "Llevar Ephirox a mi empresa" debajo del subtítulo del hero** (redundante: el CTA del topbar es sticky, siempre visible) — de paso se limpiaron `.hero-cta-row` y `.underline-link`, sin uso. Con el link fuera, se subió el `padding-bottom` de `.hero-content` (72–110px → 90–130px) para que el subtítulo (ahora último elemento) no quedara pegado al resto de la foto. Ronda aparte de ajustes solo-mobile (sin tocar escritorio): line-height del h1 (1.15→1.2) y del lead (→1.5), con más margen explícito entre kicker/h1/lead/card — pedido mientras se estaba verificando el cambio anterior, mismo turno. Commit `6407ae7`.
- **Kicker del hero bajado de tono** (turno aparte, 2026-09-16): pedido "que el título tenga el protagonismo principal" — `font-weight` 500→400 y color de `#caa057` sólido a `rgba(202,160,87,0.62)`, mismo dorado pero más apagado, para que se lea como apoyo del H1 en vez de competir con él. Commit `23f0eb0`. De paso, el `next dev` local llevaba corriendo sin responder desde el jueves (varios PID zombis en :3000) — se mató y se relanzó limpio para poder verificar con CDP.

## Resumen Ejecutivo — Sesión 2026-09-18 → 2026-09-24 — Admin de Protocolos, rediseño completo de Evolution (3 fases + "Reporte de mi equipo"), y rediseño del Hero/login/footer de la landing

### 1. Admin de Protocolos y módulo Stress
- **Protocolos (admin):** los campos Frecuencia/Duración pasan al final de "Recursos del protocolo"; el Baseline queda en su propia card antes de "Asignar a clientes activos"; "Datos del protocolo" es editable (botón "Editar datos del protocolo", `updateProtocolDetails`) y cada protocolo se colapsa en acordeón. Commit `d1c3bc0`.
- **Stress (cliente):** título "Tu protocolo activo" (grande) reemplaza a "Tu plan de regulación".

### 2. Rediseño de Evolution (spec 27–30 de `docs/prompt-ajustes-flujo-narrativo.md`, mockup `docs/evolution-module-mockup.html`) — hecho por fases
- **Fase 1 (`1978ffa`):** librería de gráficas SVG propia en `apps/web/components/evolution/charts/` (Trend, BarChart, ProportionBar, RangeBar, Donut, Legend, KpiTile/StatRow/ChartCard, CategorySection/CategoryFilterBar, Tooltip). Se eliminaron las cards viejas (frase motivacional, Panorama general, Adherencia de entrenamiento, Medidas corporales). Nuevo endpoint `GET /api/clients/:id/wellness-index/history`. Tokens de color nuevos en `tema.css` (`--eph-good/warn/low`, `--eph-pillar-*`, `--eph-sleep-stage-*`). Campo de admin en `AdminClientDetail`: checkbox "puede ver Reporte de mi equipo" (`permissions.reporteEquipo`).
- **Fase 2 (`ac2b7fb`):** categorías Stress / Sleep / Recuperación. Se omitió "Movimiento diario" (no hay umbral de pasos validado en el backend).
- **Correcciones del punto 30 (`8b86259`):** alto fijo + `preserveAspectRatio="none"` en `Trend`/`BarChart`; estado vacío explícito dentro de `Trend` (menos de 2 puntos); texto de contexto de `--eph-faint` a `--eph-muted`; "Tu típico" se desplaza cuando choca con el valor final.
- **Fase 3 (`03993ff`) — "Reporte de mi equipo":** columna nueva `clients.cohort_leader_id` (FK autorreferenciada; migración `apps/api/drizzle/manual-migrations/2026-09-19-evolution-cohort.sql`, corrida en TEST y prod). `evolution-cohort.service.ts` agrega y anonimiza ("Miembro N", nunca nombres), solo miembros con consentimiento de investigación (`getDataResearchConsentStatus`); riesgo = por debajo de su propio típico de Capacidad de regulación o recovery < 66 (mismo umbral del backend). `GET /api/clients/:id/cohort-report` re-verifica `permissions.reporteEquipo === true` en el servidor (no solo oculta la pestaña). `PATCH /api/clients/:id/cohort-leader` (admin). Pestaña "Reporte de mi equipo" en `ClientEvolutionPanel`; selector "Pertenece al equipo de" en el admin del cliente. Fuera de alcance por no inventar umbrales: Sleep/Training/Nutrición como señal de riesgo y el dashboard por pilar con Nutrición.
- **Datos de ejemplo y 2 bugs reales (`bc9ea8a`):** se sembró en la base real (la misma que dev) un equipo demo: líder "Cliente Demo" (`demo-cliente@latribu.test`, con `reporteEquipo`), miembros "Mentoring Demo" (en declive → en riesgo) y "Equipo Demo — Miembro B/C" (`*@latribu.test`), con 60 días de wearable, Capacidad de regulación, Índice de rendimiento, InBody y 3 laboratorios con Edad biológica. **Se activó `REGULATION_CAPACITY_ENABLED_IN_PRODUCTION = true`** (estaba apagado hasta aprobación clínica de los pesos 50/30/20) a pedido explícito de Alejandro para ver Stress poblado — revertir si se quiere volver a esperar esa aprobación. Bugs: (a) el reporte de cohorte reutilizaba `getRegulationCapacityOverview`, que trunca a 14 días → ahora acepta `days`; (b) `horaDormir/horaDespertar` son timestamptz completos, no "HH:MM" → el KPI mostraba el ISO crudo y la tendencia de hora de despertar salía vacía.

### 3. Landing, login y SEO (Hero, footer, secciones)
- **Hero:** el formulario del final de la landing se movió al Hero como campos compactos (Email + WhatsApp + "Solicitar una Demo"); al enviarlos abre un modal (`LeadModal`/`LeadForm`) con Nombre de la empresa, Email, WhatsApp, Cantidad de cohorte y Sitio web (guarda en `enterprise_leads`; el backend exige `nombre`, se envía el nombre de la empresa). Botón "Hablá con nuestro equipo" en el topbar y botón fijo "Chatea con nosotros" abren `ChatWidget` (opciones que llevan a WhatsApp `573214973677` = `COACH_WHATSAPP_NUMBER`). Kicker "BIENESTAR ÉLITE PARA FOUNDERS Y C‑LEVELS" debajo del H1, más pequeño. En mobile la tarjeta de estadísticas fluye bajo el formulario (no se traslapa).
- **Login:** mismo slogan que el Hero (Jost, mayúsculas, una sola línea), se quitó "Sistema de Optimización Ejecutiva", enlace "Solicitar una Demo" (apunta a la raíz de ephirox.com; el ancla `#llevarlo` ya no existe).
- **SEO:** title "Ephirox — Bienestar élite con IA para Founders y C-Levels", description nueva, canonical, imagen OG alineada (`opengraph-image.tsx`). El 301 de `/landing` y el `noindex` de `app.ephirox.com` ya existían en `middleware.ts`.
- **Secciones:** se eliminó el bloque "No es falta de disciplina. Es tu punto ciego", "DÍA 90" (con `Dia90Rail.tsx`/`HITOS`) y el carrusel "¿Cómo opera el sistema?" (`DifCarousel.tsx`/`DIF`); "Lo que cambia" con 5 filas nuevas y más compacta; footer con frase de marca, sin tagline "El lujo de tener salud." ni botón Contactar, columna COMPAÑÍA fusionada en PROGRAMA ("Solicitar una Demo"), Términos/Privacidad a la izquierda para no chocar con el botón de chat. Varios ajustes de copy ("duermes fatal", "El cuerpo te pasa factura", paso 03 con IA). Commits `039c6c7` … `eee1661`.

### 4. Aprendizajes de la sesión
- **Correr `next build` con `next dev` activo en el mismo `apps/web` rompe el build** (`Cannot find module for page: /_not-found`): parar el dev server, borrar `.next`, construir, y relanzar. El primer intento también puede fallar por un error transitorio de `next/font` (descarga de Google Fonts) — reintentar.
- **La suite completa de la API tarda ~17–19 min** (cientos de round-trips a la base remota) y su salida no aparece hasta terminar; en el frontend, los `wizard-shell-*.test.tsx` fallan por timeouts bajo carga completa y pasan aislados (mismo patrón ya documentado).
- **Tests con SWR:** usar `renderWithSWR` (`apps/web/test/swr-test-utils.tsx`) — la caché global entre tests hace que el mismo key devuelva datos de otro test.
- **`vi.mock('../lib/X')` autogenerado devuelve `undefined`:** todo componente que empiece a llamar una función nueva de ese módulo rompe los tests existentes (`.catch` de undefined) hasta agregar el `mockResolvedValue`.
- **`PATCH /api/clients/:id/permissions` REEMPLAZA el jsonb completo** (no hace merge): el cliente debe mandar los permisos existentes + el cambio.
- **jsdom no implementa `SVGGeometryElement.getTotalLength()`:** `useDrawIn` lo guarda con `typeof … !== 'function'`.
...
### Actividad 27 — Confirmar en un navegador/celular real todo el trabajo de landing de esta sesión (actualizada 2026-09-13, parcialmente en curso)

- Todo el trabajo de la sesión 2026-09-12→13 (nueva sección "Optimización personalizada", tramo "ya lo intentaste"/"coste para la empresa" con la costura visual, cards 5X/10-40%/10X realineadas, el rediseño completo de "El reporte que le llevas a tu Junta" con índice+panel fijo y 5 gráficos distintos, el fondo oscuro de `PasosMobileList` en mobile, el índice de Junta compactado en mobile, el header con ícono de login, y el formulario "Llevar Ephirox a mi empresa" rediseñado) se verificó exhaustivamente con Chrome headless vía CDP en 390/768/1440px. **Alejandro ya probó el formulario en un iPhone real** (sección 10 de esta continuación) y eso encontró 2 bugs reales que el CDP no mostraba (autofill amarillo de Safari en Celular, Correo apretado con un correo real más largo que el placeholder de prueba) — ya corregidos. Sigue pendiente que confirme en su propio dispositivo el resto: que los carruseles de "Optimización personalizada" y de las cards de coste responden bien al swipe táctil real, que la costura entre las dos secciones y el índice de Junta se ven bien, que las fotos de las 3 cards de coste (incluida `costo-senior.jpg`, con el filtro de brillo extra) se ven con tonalidad consistente entre sí, y que el nuevo índice de Junta (botones reales, sin acordeón, compactado en mobile) es cómodo de usar con el dedo.

### Actividad 28 — Revisar el desborde del botón del header a 320px (nueva, 2026-09-15, menor)

- Verificando el fix del wrap del kicker del hero a 320px (iPhone SE 1ª gen — el ancho real más angosto, no probado antes en esta sesión, que solo había cubierto 375/390px), se notó que el pill "Llevar Ephirox a mi empresa" del header se corta contra el borde derecho de la pantalla. Flagueado de pasada, no es parte de ningún pedido de esta sesión — confirmar si vale la pena arreglarlo (probablemente muy pocos usuarios reales en un dispositivo tan angosto) antes de tocarlo.

---

### Actividad 29 — Publicar y refrescar SEO/previsualizaciones de la landing (nueva, 2026-09-24)
Tras el deploy: pedir indexación de `https://ephirox.com` en Search Console (Inspección de URLs → Solicitar indexación) y refrescar la caché de previsualizaciones con Facebook Sharing Debugger ("Volver a extraer") y LinkedIn Post Inspector. Pendiente además: convertir el +57 321 497 3677 a WhatsApp Business (nombre "Ephirox" + logo) para que el chat no muestre el nombre personal.

### Actividad 30 — Revisar en navegador real Evolution (categorías, "Reporte de mi equipo") y la nueva landing (nueva, 2026-09-24)
Iniciar sesión con `demo-cliente@latribu.test` → Evolution → pestaña "Reporte de mi equipo". Revisar que el Hero, el modal de demo, el chat y "Lo que cambia" (¿cabe en una vista?) se vean bien en desktop y mobile. Decidir: (a) si `REGULATION_CAPACITY_ENABLED_IN_PRODUCTION` se queda en `true`; (b) si se limpian los datos demo (clientes `*@latribu.test`) de la base real; (c) reescritura de las dos menciones de IA en `legal-content.js` (Privacidad §4 "modelos internos de recomendación", Términos §7 "inteligencia artificial") — cualquier cambio exige subir `DATA_POLICY_VERSION`/`TERMS_VERSION`; (d) los 6 archivos de `docs/` que aparecen borrados en `git status` sin que nadie los borrara a propósito (`00-PROMPT-CORTO.md`, `EPHIROX-spec-temas.md`, `design-system-oura-brief.md`, `errors-resueltos.md`, `spec-migracion-frontend.md`, `spec-punto-ciego.md`): restaurar o confirmar el borrado.
...
- **Verificando la landing con varios procesos de Chrome headless (uno por breakpoint/ronda) vía CDP, los scripts que buscan la pestaña por `url.includes('/landing')` a veces enganchan una pestaña vieja en vez de la recién creada, y leer el DOM justo después de un `.click()` en React a veces captura el estado previo (el re-render aún no corrió)** (sesión 2026-09-13, panel de Junta directiva) — el síntoma es el mismo en ambos casos: la captura o el texto extraído corresponden al punto/estado anterior, no al que se acaba de interactuar. Mitigación que funcionó de forma consistente: matar TODOS los procesos de Chrome headless entre rondas de verificación (`pkill -f "remote-debugging-port=94"`, o el rango de puertos que se esté usando) en vez de dejar varios corriendo en paralelo, y esperar ≥500ms después de un `.click()` antes de leer texto o tomar captura, nunca encadenar click+scroll+captura sin pausas intermedias.
- **Un pedido de "diseño similar a X" puede evolucionar en varios turnos — no asumir que la primera interpretación es la final** (sesión 2026-09-13, card "¿Cómo opera el sistema?") — el primer pedido de "similar pero no igual" al panel de Junta se interpretó como igualar solo `border-radius`/grosor de borde, conservando el color cálido propio de la card; un turno después, Alejandro aclaró que quería el tono efectivamente oscuro. Confirmar visualmente en cada ronda de feedback antes de dar un ajuste de diseño por cerrado, en vez de asumir que la interpretación inicial cubre el pedido completo.
- **Safari/iOS pinta su propio fondo amarillo sobre cualquier `<input type="tel">`/`type="email"` apenas se empieza a escribir, sin que el usuario haya aceptado ningún autofill** (sesión 2026-09-13, formulario "Llevar Ephirox a mi empresa", encontrado por Alejandro en un iPhone real — Chrome headless/CDP no lo reproduce). Fix estándar: `input:-webkit-autofill { -webkit-text-fill-color: <color de texto>; box-shadow: 0 0 0 1000px <color de fondo de la tarjeta> inset; -webkit-box-shadow: 0 0 0 1000px <mismo color> inset; transition: background-color 9999s ease-in-out 0s; }` — el inset debe ser un color opaco que iguale el fondo real donde vive el input (no `transparent`, que no logra tapar el pintado interno de WebKit). Cualquier input `tel`/`email` nuevo en esta landing debería llevarlo desde el principio.
- **Medir el ancho mínimo de una columna contra el placeholder de diseño no es lo mismo que medirlo contra lo que un usuario real teclea** (sesión 2026-09-13, mismo formulario) — "nombre@empresa.com" (placeholder, 19 caracteres) cabía con margen en la columna calculada, pero el correo real de Alejandro (24 caracteres) se veía recortado. Para campos de longitud variable por naturaleza (correo, nombre, cualquier campo libre largo) es más robusto dejarlos a ancho completo en mobile que perseguir el ajuste exacto basado en el placeholder de ejemplo — reservar el layout de 2 columnas para campos con longitud más acotada (rol, tamaño de empresa, etc.).
- **Quitar un campo de un formulario público de esta landing que tenga columna `NOT NULL` en Postgres rompe el envío real, no solo la UI** (sesión 2026-09-15, formulario "Llevar Ephirox a mi empresa" al simplificarse de "propuesta" a "demo") — el zod schema compartido (`@latribu/shared-types`) exigía `empresa`/`rol`, y la columna real de la tabla también. Antes de borrar un campo de un formulario que ya tiene datos en producción, revisar en orden: (1) el zod schema del input en `packages/shared-types/src/*.ts`, (2) la definición de columna en `apps/api/src/models/schema.ts` (¿`notNull()`?), (3) cualquier código que arme un mensaje/email asumiendo que ese campo siempre viene (`enterprise-leads.service.ts` armaba el asunto del correo y la alarma admin directamente con `lead.empresa`, sin chequeo). Mismo patrón ya documentado de sesiones anteriores para cambios de schema: nunca `drizzle-kit push`, script `tsx`/`postgres` desechable con el DDL a mano contra `DATABASE_URL` y `TEST_DATABASE_URL`, confirmación explícita del usuario (el clasificador de auto mode lo bloquea por defecto), y probar con un envío real contra la API antes de dar el cambio por bueno — no solo con `tsc`.
- **Un elemento nuevo agregado como hijo de un `display:grid` con `gap` parejo entre hijos (ej. `.hero-content`, kicker+h1+lead) hereda ese mismo gap grande hacia el hijo siguiente, aunque visualmente necesite quedar mucho más pegado (un kicker junto a su título)** (sesión 2026-09-13→15, kicker del hero) — en vez de restructurar el grid o mover el elemento fuera de él, un `margin-bottom` negativo del tipo `calc(<gap deseado> - <mismo gap del grid>)` cancela el gap heredado y deja exactamente el espacio chico querido, sin tocar el espaciado entre el resto de los hermanos. Hay que repetir la misma expresión de `clamp()`/`vw` del `gap` del grid dentro del `calc()` para que se mantenga correcto en todos los anchos, no solo en el que se probó.
