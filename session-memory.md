...
- **El PDF de nutrición mostraba el texto plano "Ephirox" en vez del logo real.** La lógica real de generación vive inline en `ClientNutritionPanel.tsx` (`window.open` + `document.write` + `print()`) — se confirmó en el proceso que `NutritionPdfGenerator.tsx` es código muerto, nunca importado en ningún lado. Se incrustó el SVG real de `Documentos/Brand/ephirox-lockup-horizontal-oro.svg` en el header del PDF. **Pendiente de decisión, no corregido a ciegas:** el wordmark de esa variante usa relleno `#F5F1E8`, que da ~1.1:1 de contraste contra el fondo claro del PDF (`#EDE6DC`) — casi ilegible impreso. Existe una variante "negro" (`#0B0A08`) en la misma carpeta que se vería mucho mejor ahí; falta que Alejandro confirme el cambio.
...
### Actividad 8 — Decidir la variante del logo en el PDF de nutrición: "oro" vs "negro" (nueva, 2026-09-01)

- Ver sección 5 del resumen de esta sesión: el wordmark de `ephirox-lockup-horizontal-oro.svg` (relleno `#F5F1E8`) queda con contraste ~1.1:1 sobre el fondo claro del PDF (`#EDE6DC`), casi ilegible impreso. La variante "negro" (`#0B0A08`, misma carpeta de `Documentos/Brand/`) se vería mucho mejor ahí. No se cambió a ciegas — falta que Alejandro confirme cuál variante usar.

### Actividad 9 — Confirmar si se quiere commitear los cambios pendientes de `WizardShell.tsx`/`TrainingHome.tsx`/`TrainingShell.tsx`/`training-client.ts` (arrastrada de una tarea anterior)
